...
import { Upload, Download, FileSpreadsheet, Check, AlertCircle, X, Clock, ArrowLeft } from 'lucide-react'
...
function SpreadsheetUpload() {
  ...
  const [cellErrors, setCellErrors] = useState([])   // [{row_index, column, message}]
  ...
  const handleCellEdit = (rowIndex, column, value) => {
    ...
    setValidated(false)   // any edit invalidates the last validation
    setCellWarnings([])
  }
        ...
        <div style={{
          ...
          <div>
            ...
            <div style={{ fontSize: '12px', color: theme.colors.textMuted, fontFamily: theme.fonts.primary, marginTop: '2px' }}>
              Upload pre-assigned club data via spreadsheet
            </div>
          </div>
          <button
